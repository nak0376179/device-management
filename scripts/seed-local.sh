#!/usr/bin/env bash
# Seed test data into Floci DynamoDB and update device config.json.
# Group: dev-group / devpass, Device: virtual-device-01
set -euo pipefail
...
DEVICE_RESP=$(curl -s -X POST "$BACKEND_URL/api/admin/groups/dev-group/devices" \
  -H "Content-Type: application/json" \
  -d '{"thing_name":"virtual-device-01"}' 2>/dev/null || echo "{}")
...
if [[ -f "$CONFIG" && -n "$API_KEY" ]]; then
  python3 - "$CONFIG" "$API_KEY" "$BACKEND_URL" <<'PY'
import sys, json
path, api_key, backend_url = sys.argv[1], sys.argv[2], sys.argv[3]
cfg = json.loads(open(path).read())
cfg["api_key"] = api_key
cfg["backend_url"] = backend_url
open(path, "w").write(json.dumps(cfg, indent=2) + "\n")
print(f"Updated {path}: api_key={api_key}")
PY
fi
