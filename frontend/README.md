...
| `npm test` | Vitest 全実行（unit + storybook の2プロジェクト） |
| `npm run test:unit` | ユニットテストのみ（Node, 高速） |
| `npm run test:storybook` | 全ストーリーをブラウザでテスト（play 含む） |
| `npm run test:watch` | Vitest をウォッチモードで実行 |

### テスト構成

- **unit**（`src/**/*.test.ts`, Node 実行）: 純粋ロジックの単体テスト。`cn`（`lib/utils`）・`isTerminal`（`types`）・`api`（`fetch`/`localStorage` をモック）のサンプルあり。
- **storybook**（Chromium 実行）: 各ストーリーをレンダリングしてスモークテスト。`play` を持つストーリー（`Button`/`StatusBadge`/`DeviceList`/`LoginForm`）はインタラクションテストとして実行されます。
